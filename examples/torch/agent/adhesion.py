...
def random_3d_unit_vectors(n_vectors: int, device: str = "cpu") -> torch.Tensor:
    ...
    device : str
        The device to allocate the tensor on. Default is "cpu".
    ...
    """
    vectors = (torch.rand((n_vectors, 3), device=device) * 2) - 1
...
def compute_forces(
    positions: torch.Tensor,
    cell_adhesion_radii: torch.Tensor,
    cell_adhesion_strength: torch.Tensor,
    cell_repulsion_radii: torch.Tensor,
    cell_repulsion_strength: torch.Tensor,
    boundary_adhesion_radii: torch.Tensor,
    ...
    locomotion_direction_change_probability = 0.01 * torch.ones(
        (n_agents,), device=positions.device
    )
    locomotion_forces, updated_locomotion_direction = biased_random_locomotion_force(
        ...
        bias_direction=torch.zeros((n_agents, 3), device=positions.device),
        bias_constant=torch.zeros((n_agents,), device=positions.device),
...
if __name__ == "__main__":
    ...
    locomotion_direction = random_3d_unit_vectors(n_cells, device=device)
    locomotion_speed = 0.3 * torch.ones((n_cells,), device=device)  # µm/s

    # cell-cell adhesion force parameters
    cell_adhesion_radii = 1.25 * cell_radius * torch.ones((n_cells,), device=device)
    cell_adhesion_strength_field_array = min_cell_adhesion * torch.ones(
        total_shape, device=device
    )
    ...
    for x_index in range(domain_lower_left[0], domain_upper_right[0]):
        cell_adhesion_strength_field_array[x_index:, :, :] = (
            min_cell_adhesion + cell_adhesion_slope * (x_index - domain_lower_left[0])
        )
    cell_adhesion_strength_field = ScalarField(field=cell_adhesion_strength_field_array)

    # cell-cell repulsion force parameters
    cell_repulsion_radii = cell_radius * torch.ones((n_cells,), device=device)
    cell_repulsion_strength = 1.0 * torch.ones((n_cells,), device=device)

    # cell-boundary adhesion force parameters
    boundary_adhesion_radii = cell_radius * torch.ones((n_cells,), device=device)
    boundary_adhesion_strength_field_array = min_boundary_adhesion * torch.ones(
        total_shape, device=device
    )
    ...
    for x_index in range(domain_lower_left[0], domain_upper_right[0]):
        boundary_adhesion_strength_field_array[x_index:, :, :] = (
            min_boundary_adhesion
            + boundary_adhesion_slope * (x_index - domain_lower_left[0])
        )
    boundary_adhesion_strength_field = ScalarField(
        field=boundary_adhesion_strength_field_array
    )

    # cell-boundary repulsion force parameters
    boundary_repulsion_radii = cell_radius * torch.ones((n_cells,), device=device)
    boundary_repulsion_strength = 2.0 * torch.ones((n_cells,), device=device)
    ...
    for time_step_index in trange(int(n_time_steps)):
        ...
        cell_adhesion_strength = cell_adhesion_strength_field.sample(positions)
        boundary_adhesion_strength = boundary_adhesion_strength_field.sample(positions)
