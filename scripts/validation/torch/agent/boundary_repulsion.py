"""Script to validate the boundary repulsion force.

There is an agent that starts far away from the boundary
moves towards it with fully directed locomotion. It should
stop when the locomotion force and repulsion force are equal
and opposite.
"""
...
if __name__ == "__main__":
    ...
    # run parameters
    device = "cpu"
    plot_path = "boundary_repulsion_positions.png"
    ...
    repulsion_strength = torch.tensor([2])
    ...
    for time_step_index in range(n_time_steps):
        forces = compute_forces(
            ...
            boundary_repulsion_strength=repulsion_strength,
    ...
    axs[0].axhline(7.5, c="gray", label="expected")
    axs[0].scatter(logged_times, logged_positions[:, 0], s=0.1, label="actual")
