...
class SoapBubble(torch.nn.Module):
    ...
    def forward(self, mesh_data):
        ...
        for _ in range(self.n_forward_iterations):
            # print(f"iteration {i} / {self.n_forward_iterations}")
...
if __name__ == "__main__":
    ...
    # each element is a (n_cell_face, 3) array of indices for the vertices of the faces
    cell_face_list = [initial_faces[cell_faces, :] for cell_faces in face_cell_index]
    mesh_data = {
        "vertices": initial_vertices,
        "faces": initial_faces,
    ...
    print(f"simulation took {simulation_time/n_forward_iterations:.4f} s/iteration")
